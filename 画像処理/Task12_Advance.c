...
int main(void){
  ...
  char outputpath[256] = "./Output/Task12/Color_Labeling.ppm";    // 出力するディレクトリを宣言
  int i, j;
  ...
  for(i = 0; i < H; i++){
    for(j = 0; j < W; j++){
      if(labeling[i][j] != -1){
        rgb[i][j].red = (30 * (labeling[i][j] + 15)) % 255;
        rgb[i][j].green = (20 * (labeling[i][j] + 15)) % 255;
        rgb[i][j].blue = (10 * (labeling[i][j] + 10)) % 255;
      }
      else{
        rgb[i][j].red = 0;
        rgb[i][j].green = 0;
  ...
  fwrite(rgb, sizeof(struct RGB), W * H, fp);                   // ファイルにデータ部を書き込む
  fclose(fp);                                                   // ファイルを閉じる
